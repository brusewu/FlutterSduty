...
class UnitConverterState extends State<UnitConverter>{
  ...
  Future<void> updateConversion() async{
    if(widget.category.name == apiCategory['name']){
      final api = Api();
      final conversion = await api.convert(apiCategory['route'], inputValue.toString(),
          fromValue.name, toValue.name);
      setState(() {
        convertedValue = format(conversion);
      });
    }else{
      setState(() {
        convertedValue = format(
          inputValue * (toValue.conversion / fromValue.conversion)
        );
      });
    }
  }
        ...
        children: <Widget>[
          TextField(
            ...
            decoration: InputDecoration(
              ...
              errorText: showValidationError ? 'Invalid nmber entered' : null,
              labelText: 'Input',
